  ...
  public void showInfo(ResultsTable rt, int row){
      ...
      if (vData.size() > 0) {
       ...
       if ((measureFlag & SLICE) != 0){
        ...
        if ((measureFlag & BASIC) != 0){
          rt.setValue("Blob", row, (double) label);
          rt.setValue("Count", row, 0);
          rt.setValue("Area", row, Double.NaN);
          rt.setValue("Perim", row, Double.NaN);
        }
       if ((measureFlag & SLICE) != 0){
  		   rt.setValue("Slice", row, (double) slice);
  		 }
  		  if ((measureFlag & LOCATION) != 0) {
    		  rt.setValue("BX", row, Double.NaN);
    		  rt.setValue("BY", row, Double.NaN);
    	 	  rt.setValue("BW", row, Double.NaN);
    		  rt.setValue("BH", row, Double.NaN);
  		      rt.setValue("XC", row, Double.NaN);
  		      rt.setValue("YC", row, Double.NaN);
    		  rt.setValue("XStart", row, Double.NaN);
    		  rt.setValue("YStart", row, Double.NaN);
    		  rt.setValue("Side", row, Double.NaN);
  		  }
  		  	  
  		 // rt.setValue("Hferet", row, hferet);
  		 // rt.setValue("Vferet", row, vferet);
  		 if ((measureFlag & SHAPE) != 0)
          rt.setValue("FeretMax", row, Double.NaN);
          rt.setValue("FeretMin", row, Double.NaN);
          rt.setValue("FeretMean", row, Double.NaN);
          rt.setValue("FeretMed", row, Double.NaN);
          rt.setValue("FeretAngle", row, Double.NaN);
          rt.setValue("FeretX", row, Double.NaN);
          rt.setValue("FeretY", row, Double.NaN);
  		  rt.setValue("Theta", row, Double.NaN);
  		  rt.setValue("Orient", row, Double.NaN);
  		  rt.setValue("Circ", row, Double.NaN);
  		  rt.setValue("Round", row, Double.NaN);
  		  rt.setValue("Solid", row, Double.NaN);
  		  rt.setValue("Compact", row, Double.NaN);
  		  rt.setValue("Extent", row, Double.NaN);
  		  rt.setValue("Aspect", row, Double.NaN);
  		  rt.setValue("VoidFraction", row, Double.NaN);
  		  rt.setValue("NEuler", row, Double.NaN);
  		 }

  		 if ((measureFlag & ELLIPSE) != 0){
    		  rt.setValue("Semimajor", row, Double.NaN);
    		  rt.setValue("Semiminor", row, Double.NaN);
  	   	  rt.setValue("Ecc", row, mecc);
  		 }
  		   		 
  		 if ((measureFlag & VOLUME) != 0) {
  		  rt.setValue("Vol", row, Double.NaN);
  		  rt.setValue("NTricky", row, Double.NaN);
  		 }//vol
  		 
  		 if ((measureFlag & GRAY) != 0) {
  		    rt.setValue("GMin", row, Double.NaN);
  		    rt.setValue("GMax", row, Double.NaN);
  		    //rt.setValue("GMass", row, gmass);
  		    rt.setValue("GMean", row, Double.NaN);
  		    rt.setValue("GVar", row, Double.NaN);
  		    rt.setValue("GSkew", row, Double.NaN);
  		    rt.setValue("GKurt", row, Double.NaN);
  		  
  		 }
  		 
  		 if ((measureFlag & MOMENTS) != 0) {
  		  for (int k = 0; k < nHu; k++){rt.setValue("Hu" + (k+1), row, Double.NaN);}  		 
  		  for (int k = 0; k < nAMI; k++){ rt.setValue("AMI" + (k+1), row, Double.NaN);}
  		 }  		 
      }
  }
